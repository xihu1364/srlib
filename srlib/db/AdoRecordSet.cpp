...
BOOL CAdoRecordSet::Open(LPCTSTR strSQL, long lOption, CursorTypeEnum CursorType, LockTypeEnum LockType)
{
	...
	catch (_com_error e)
	{
		...
		TRACE(_T("Warning: CAdoRecordSet::Open 方法发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), strMsg.GetBuffer(), __FILE__, __LINE__);
		return FALSE;
	}
}
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName, char* value)
{
	CString strTemp;

	GetCollect(strFieldName, strTemp);

	strcpy(value, strTemp.GetBuffer(0));
